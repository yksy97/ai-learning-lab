...
let gan, ar, hist, viewZ, crn, realRef, ganSamples, arSamples;
...
function reset() {
  ...
  hist = new History(['pgan', 'rgan', 'par', 'rar'], 400);
...
function measure() {
  ...
  hist.push({ step: gan.step, pgan: g.precision, rgan: g.recall, par: a.precision, rar: a.recall });
...
function setRunning(v) {
  ...
  if (!v && gan) { refreshAR(); refreshGAN(true); measure(); render(); }
}
...
function init() {
  ...
  $('showBg').addEventListener('change', (e) => { state.show.bg = e.target.checked; refreshGAN(true); render(); });
  ...
  $('btnStep').addEventListener('click', () => { gan.trainStep(); ar.trainStep(); setRunning(false); });
  ...
  matchMedia('(prefers-color-scheme: dark)').addEventListener('change', () => { refreshAR(); refreshGAN(true); render(); });
