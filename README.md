...
> **AI の「なぜ？」が、見えてくる。**
> AI を使うだけで終わらせず、動かして・予想して・壊して、自分の言葉で説明できるようになるための学習アプリです。

生成モデル（GAN・VAE・自己回帰 Transformer）と RAG を、それぞれ **たとえ話ガイド**（順を追って読む）と **詳細**（自由に設定を変えて試す）の2枚で見られます。
学習はブラウザの中でその場で走り、外部のサービスにもサーバにも、データは送られません。

ニューラルネットワークの順伝播・逆伝播・Adam、Transformer の注意機構、TF-IDF 検索まで、すべて素の JavaScript で書いてあります。
**npm install は不要**で、外部ライブラリにもサーバ側の計算にも依存しません。

### なぜ JavaScript で書いてあるのか

実務で AI を書く言語は Python です。このアプリを JavaScript で書いているのは、**教材として「開いた瞬間に動き、つまみを動かした瞬間に結果が変わる」ことを優先した**ためで、Python より良いからではありません。
そのぶん、**教材の中に出てくるコードはすべて Python（PyTorch / numpy）を主**にしてあります。各詳細ページの「コードで見る」では、実務で書く PyTorch のコードを左に、同じ処理をこの画面の中で書いたものを右に並べています。
...
たとえ話ガイドには各ステップに「専門用語では」の対応表が付いています。はじめて触るモデルは、ガイドから見てください。
