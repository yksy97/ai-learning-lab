...
| 共通のしくみ | 4つのモデルを「入力 → 表現 → 計算 → 出力 → 評価 → 学習 → 推論」の同じ枠に並べる |
...
- **共通のしくみ**：4つのモデルを同じ7段の枠に並べた表です。段をクリックすると「共通していること」と「分かれること」が出ます。モデルを別々に覚えるのではなく、骨格の同じところと違うところで捉えられるようにするためのページです
